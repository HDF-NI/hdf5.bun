...
import { hdf5_hl } from "../hdf5_hl.js";
import globs from '../globals.js';

export function isImage(fileId, dsetName) {
    let status = hdf5_hl.H5IMis_image(fileId, dsetName);
    if (status < 0) {
...
export function makeImage(fileId, dsetName, buffer, options) {
    ...
    let status = hdf5_hl.H5LTmake_dataset(fileId, dsetNamePtr, 3, dims, hdf5.H5T_NATIVE_UCHAR, buffer);
    console.log("H5LTmake_dataset status: "+status);
    if (status < 0) {
        throw new Error("failed to make image");
    }
    status = hdf5_hl.H5LTset_attribute_string(fileId, dsetNamePtr, "CLASS", "IMAGE");
    status = hdf5_hl.H5LTset_attribute_string(fileId, dsetNamePtr, "IMAGE_SUBCLASS", "IMAGE_BITMAP");
    status = hdf5_hl.H5LTset_attribute_string(fileId, dsetNamePtr, "IMAGE_SUBCLASS", "IMAGE_TRUECOLOR");
    status = hdf5_hl.H5LTset_attribute_string(fileId, dsetNamePtr, "IMAGE_VERSION", "1.2");
    if (interlace) {
        status = hdf5_hl.H5LTset_attribute_string(fileId, dsetNamePtr, "INTERLACE_MODE", interlace);
        if (status < 0) {
            throw new Error("failed to make image");
        }
    }
...
export function readImage(fileId, dsetName, cb) {
    ...
    const dsetNamePtr = Buffer.from(dsetName + "\0");
    let status = hdf5_hl.H5IMget_image_info(fileId, dsetNamePtr, width, height, planes, interlace, npals);
    if (status < 0) {
        throw new Error("failed to get image info");
    }
    const w = width[0];
    ...
    status = hdf5_hl.H5LTread_dataset(fileId, dsetNamePtr, fileDataTypeId, contentBuffer);
